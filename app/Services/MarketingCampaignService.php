...
class MarketingCampaignService
{
    ...
    {
        $rows = DB::table('marketing_campaigns')->orderBy('created_at', 'desc')->orderBy('id', 'desc')->get();

        return $rows->map(fn ($row) => $this->formatCampaign((array) $row))->toArray();
    }
    ...
    public function getCampaign(int $id): array
    {
        ...
        return $this->formatCampaign((array) $row);
    }
    ...
    {
        $affected = DB::table('marketing_campaigns')->where('id', $id)->delete();
        if ($affected === 0) {
            throw new RuntimeException('Campaign not found.', 404);
        }
    }
    ...
    private function runCampaignInternal(int $id, bool $dryRun, string $runType): array
    {
        ...
        $runId = DB::table('marketing_campaign_runs')->insertGetId([
            'campaign_id' => $id,
            'run_type' => $runType,
            'dry_run' => $dryRun ? 1 : 0,
        ...
        if (! $dryRun) {
            ...
            if (! empty($recipientsToInsert)) {
                ...
                foreach (array_chunk($recipientsToInsert, 500) as $chunk) {
                    DB::table('marketing_campaign_recipients')->insert($chunk);
                }
            }

            DB::table('marketing_campaign_runs')->where('id', $runId)->update(['queued_count' => $queuedCount]);
    ...
    public function analytics(int $id): array
    {
        ...
        $channelRows = DB::table('marketing_campaign_recipients')
            ->select('channel', DB::raw('COUNT(*) AS total'))
            ->where('campaign_id', $id)
            ->groupBy('channel')
            ->get();

        $runsRows = DB::table('marketing_campaign_runs')
        ...
        $statusByChannel = DB::table('marketing_campaign_recipients')
        ...
        $recentFailureRows = DB::table('marketing_campaign_recipients')
        ...
        return [
            ...
            'byStatusByChannel' => $statusByChannel->map(function ($row) {
                return [
                    'channel' => (string) ($row->channel ?? ''),
            ...
            'failureSummary' => array_map(function ($row) {
                return [
                    'channel' => (string) ($row->channel ?? ''),
            ...
            'recentFailures' => $recentFailureRows->map(function ($row) {
                return [
            ...
            'runs' => $runsRows->map(function ($row) {
                return [
                    'id' => (int) ($row->id ?? 0),
    ...
    private function audienceAbandonedCart(): array
    {
        ...
        return $this->formatAudienceRows(json_decode(json_encode($rows), true) ?: []);
    }
