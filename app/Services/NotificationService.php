 ...
 * Set config('mail.from.address', 'hello@1625autolab.com') in .env to enable notifications.
 */
class NotificationService
{
    ...
    {
        if (config('mail.from.address', 'hello@1625autolab.com') === '') {
            return;
        }
     ...
     * inbox (if config('mail.admin.address', '1625autolab@gmail.com') is set).
     *
    ...
    {
        if (config('mail.from.address', 'hello@1625autolab.com') === '' || count($recipients) === 0) {
            return;
        }
    ...
    {
        if (config('mail.from.address', 'hello@1625autolab.com') === '' || trim($staffEmail) === '') {
            return;
        }
    ...
    public function marketingCampaignMessage(
        ...
        if (config('mail.from.address', 'hello@1625autolab.com') === '' || trim($email) === '') {
            return;
        }
    ...
    public function sendWaitlistSlotAvailable(
        ...
        if (config('mail.from.address', 'hello@1625autolab.com') === '' || $email === '') {
            return;
        }
    ...
    {
        return view("emails.features.{$template}", $vars)->render();
    }
    ...
    /**
     * Send a test email to config('mail.admin.address', '1625autolab@gmail.com') (or a custom recipient).
    ...
    {
        $to = $recipient !== '' ? $recipient : config('mail.admin.address', '1625autolab@gmail.com');

        if (config('mail.from.address', 'hello@1625autolab.com') === '') {
            return [
                'sent' => false,
                'recipient' => $to,
                'reason' => 'mail.from.address is not configured in .env',
            ];
        }
        ...
        if ($to === '') {
            return [
                'sent' => false,
                'recipient' => $to,
                'reason' => 'No recipient | set mail.admin.address in config or supply a recipient',
            ];
        }
    ...
    {
        return [
            'configured' => config('mail.from.address') !== null,
            'fromSet' => config('mail.from.address') !== null,
            'adminSet' => config('mail.admin.address') !== null,
    ...
    private function send(string $to, string $toName, string $subject, string $htmlBody): void
    {
        ...
        $fromName = config('mail.from.name', '1625 Auto Lab');
        $from = config('mail.from.address', 'hello@1625autolab.com');
    ...
    {
        $parts = preg_split('/\s*,\s*/', (string) config('mail.admin.address', '1625autolab@gmail.com')) ?: [];
        $emails = [];
    ...
    private function normalizeFancyText(string $text): string
    {
        ...
        return preg_replace_callback($search, function ($match) {
            ...
            if ($code >= 0x1D5D4 && $code <= 0x1D5ED) {
                return chr($code - 0x1D5D4 + 65);
            }  // A-Z
            if ($code >= 0x1D5EE && $code <= 0x1D607) {
                return chr($code - 0x1D5EE + 97);
            }  // a-z
            if ($code >= 0x1D7EC && $code <= 0x1D7F5) {
                return chr($code - 0x1D7EC + 48);
            }  // 0-9
