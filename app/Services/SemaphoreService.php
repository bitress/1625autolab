...
class SemaphoreService
{
    ...
    public function __construct()
    {
        ...
        $this->senderName = config('services.semaphore.sender_name', env('SEMAPHORE_SENDER_NAME', '1625AutoLab'));
        $this->accountCacheTtl = max(0, (int) config('services.semaphore.account_cache_ttl', env('SEMAPHORE_ACCOUNT_CACHE_TTL', 60)));
        $this->messagesCacheTtl = max(0, (int) config('services.semaphore.messages_cache_ttl', env('SEMAPHORE_MESSAGES_CACHE_TTL', 30)));
    }
    ...
    public function getAccount(bool $refresh = false): array
    {
        ...
        if ($this->accountCacheTtl > 0) {
            Cache::put($cacheKey, $result, $this->accountCacheTtl);
        }
    ...
    public function getMessages(array $filters = [], bool $refresh = false): array
    {
        ...
        if ($this->messagesCacheTtl > 0) {
            Cache::put($cacheKey, $result, $this->messagesCacheTtl);
        }
